...
uint256 hashGenesisBlock("0x74e247e5ae19b951c22d66c40253b44552467443059e9b7ae30ab18473ba4537");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Colognecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Colognecoin Signed Message:\n";
...
{
    // Colognecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 30 * 60; // old retarget (1/2hr)
static const int64 nTargetTimespanNEW = 60 ; // CologneCoin: every 1 minute
static const int64 nTargetSpacing = 60; // CologneCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Colognecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Colognecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KSTA 20.03.2014 | Heumarkt in Köln - Greifvögel gegen die Taubenplage";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("92ebec9ab4bc25968f8e72ad708016e84eb963148c54934cc0240cf92272d02e"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ColognecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ColognecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ColognecoinMiner(CWallet *pwallet)
{
    printf("ColognecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("colognecoin-miner");
    ...
    try { loop {
        ...
        printf("Running ColognecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ColognecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ColognecoinMiner, pwallet));
}
